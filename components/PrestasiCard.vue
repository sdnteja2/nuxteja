<template>
  <div class="px-4 py-10 mx-auto space-y-8 lg:max-w-3xl">
    <div class="space-y-8">
      <div>
        <h3 class="mb-3 text-xl font-bold font-mont md:text-2xl">{{ blok.tahun }}</h3>
        <ul class="space-y-6">
          <li class="space-y-1 rounded-md ">
            <div class="flex items-center space-x-2">
              ...
              <h4 class="font-bold uppercase font-planar">
                {{ blok.peringkat }} <span> {{ blok.perlombaan }} </span>
              </h4>
            </div>
            <p class="pl-1 ml-7 font-planar text-aDark">{{ blok.deskripsi }}
            </p>
          </li>
        </ul>
      </div>
